...
void main(void)
{
  ...
  // Text
   ...
   // Portals
   int avoidPortal=rmCreateTypeDistanceConstraint("avoid portal", "well of urd", 30.0);
   ...
   rmAddObjectDefItem(portalID, "Well of Urd", 1, 0.0);
  ...
  /*    rmSetAreaCoherence(id, 0.0); */
   ...
   // portals
   rmPlaceObjectDefPerPlayer(portalID, false, 3);
   ...
   int bonusCount=cNumberNonGaiaPlayers * 5 + 5;  // num players plus some extra
   for(i=0; <bonusCount)
   {
      int bonusIslandID=rmCreateArea("bonus island"+i);
      rmSetAreaSize(bonusIslandID, rmAreaTilesToFraction(3000), rmAreaTilesToFraction(5000));
...
/*      rmAddAreaConstraint(bonusIslandID, playerEdgeConstraint); */
   ...
   for(i=1; <cNumberPlayers)
   {
      ...
      rmPlaceObjectDefInArea(bonusSettlementID, 0, rmAreaID("bonus island"+(i + 2 * cNumberPlayers)), 1);
   }
...
//      rmPlaceObjectDefInArea(farGoldID, 0, rmAreaID("player"+i), 2);
   ...
   for (i=1; < cNumberNonGaiaPlayers * 2) {
      rmPlaceObjectDefInArea(relicID, 0,  rmAreaID("bonus island"+i));
   }

   // Portals

   for (i=1; < cNumberNonGaiaPlayers * 5 + 10) {
