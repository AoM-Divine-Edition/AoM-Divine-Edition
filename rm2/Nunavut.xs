...
void main(void){
	...
	for(i=0; <2*cNumberNonGaiaPlayers*mapSizeMultiplier) {
		...
		rmAddObjectDefItem(columnID, "Inuksuk Large", rmRandInt(0,5), 0.0);
		rmAddObjectDefItem(columnID, "columns broken", rmRandInt(0,2), 4.0);
		rmAddObjectDefItem(columnID, "Inuksuk Small", rmRandFloat(1,2), 4.0);
	...
	rmAddObjectDefItem(mediumDeerID, "Moose", rmRandInt(4,6), 4.0);
	...
	if(bonusChance < 0.33) {
		...
		rmAddObjectDefItem(bonusHuntableID, "moose", rmRandInt(3,4), 4.0);
	} else {
		rmAddObjectDefItem(bonusHuntableID, "snow leopard", rmRandInt(2,3), 4.0);
	}
	...
	if(predatorSpecies<0.5) {
		rmAddObjectDefItem(farPredatorID, "ice bear", 2, 4.0);
	} else {
		rmAddObjectDefItem(farPredatorID, "ice bear", 2, 4.0);
	}
	...
	rmSetTriggerEffectParam("SetName", "Ghost Lake");
