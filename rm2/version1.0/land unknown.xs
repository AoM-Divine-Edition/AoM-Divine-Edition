...
// Main entry point for random map script
  ...
  /* ZERO STEP determine square or rectangular */
   ...
   if(terrainChance == 0) 
   {
      ...
      if(baseTerrainChance < 0.3)         /* Greek ocean */
      {
   ...
   {
      baseTerrainChance = rmRandFloat(0, 1);
      if(baseTerrainChance < 0.3)   /* Norse ocean */
      {
   ...
   {
      baseTerrainChance = rmRandFloat(0,1); 
      if(baseTerrainChance < 0.3)   /* Egyptian ocean */
      {      
  ...
  // Text
   ...
   int teamLands = 0; /* never mess with team lands, since we don't want islands */
   ...
   if (baseTerrainChance < 0.3) /* never avoid on water map */
   {
   ...
   if(baseTerrainChance < 0.3) /* base is water */
   {
      if(centerChance < 0.4)
      {
         centerType = 1; /* land */
  ...
  // Text
   ...
   if(baseTerrainChance < 0.3)   /* ocean */
   {
