...
void main(void)
{
	...
	rmxInit("Odin's Altar (by AProperGentleman)", false, false, false);
  ...
  // Text
   ...
   rmTerrainInitialize("CliffNorseA", 0.0);
   ...
   if(waterBuffaloNumber<0.3)
      rmAddObjectDefItem(closeWaterBuffaloID, "crowned crane", 1, 4.0);
   else if(waterBuffaloNumber<0.6)
      rmAddObjectDefItem(closeWaterBuffaloID, "crowned crane", 2, 4.0);
   else 
      rmAddObjectDefItem(closeWaterBuffaloID, "crowned crane", 3, 6.0);
   ...
   rmAddObjectDefItem(mediumGoldID, "gold mine", 1, 5.0);
   ...
   rmSetObjectDefMinDistance(bonusSettlementID, 80.0);
   ...
   rmAddObjectDefItem(titanID, "Odins Tower", 1, 0.0);
   ...
   int fixedPortalID=rmCreateObjectDef("fixed portal");
   rmAddObjectDefItem(fixedPortalID, "Sky Passage", 1, 0.0);
   rmSetObjectDefMinDistance(fixedPortalID, 0.0);
   rmSetObjectDefMaxDistance(fixedPortalID, 0.0);
   ...
   if(predatorSpecies<0.5)   
      rmAddObjectDefItem(farPredatorID, "elk", 4, 4.0);
   else
      rmAddObjectDefItem(farPredatorID, "elk", 8, 4.0);
   ...
   if(bonusChance<0.5)   
      rmAddObjectDefItem(bonusHuntableID, "elk", 5, 4.0);
   else
      rmAddObjectDefItem(bonusHuntableID, "elk", 5, 8.0);
...
rmSetAreaLocation(bonusIslandID, 0.5, 0.5);
    rmSetAreaSize(bonusIslandID, rmAreaTilesToFraction(4500), rmAreaTilesToFraction(4500));
    rmSetAreaTerrainType(bonusIslandID, "NorseRoadA"); 
...
int shadeID=rmCreateObjectDef("shade "+i);
   rmAddObjectDefItem(shadeID, "wolf arctic", 1, 0.0);
   rmSetObjectDefMinDistance(shadeID, 0.0);
   rmSetObjectDefMaxDistance(shadeID, 10.0);
   

rmPlaceObjectDefAtLoc(titanID, 0, 0.5, 0.5);

float theta = 0.0;
 for(i=2; < 2* realNumberPlayers)
   {
if (i % 2 == 1) {
   theta = rmDegreesToRadians((360 / (2*realNumberPlayers -2)) * i);
   rmPlaceObjectDefAtLoc(vaultID, 0, 0.5+cos(theta)*rmAreaTilesToFraction(sqrt(realNumberNonGaiaPlayers)*4500), 0.5+sin(theta)*rmAreaTilesToFraction(sqrt(realNumberNonGaiaPlayers)*4500));
   rmPlaceObjectDefAtLoc(shadeID, 0, 0.5+cos(theta)*rmAreaTilesToFraction(sqrt(realNumberNonGaiaPlayers)*4500), 0.5+sin(theta)*rmAreaTilesToFraction(sqrt(realNumberNonGaiaPlayers)*4500));
   rmPlaceObjectDefAtLoc(shadeID, 0, 0.5+cos(theta)*rmAreaTilesToFraction(sqrt(realNumberNonGaiaPlayers)*4500), 0.5+sin(theta)*rmAreaTilesToFraction(sqrt(realNumberNonGaiaPlayers)*4500));
   rmPlaceObjectDefAtLoc(shadeID, 0, 0.5+cos(theta)*rmAreaTilesToFraction(sqrt(realNumberNonGaiaPlayers)*4500), 0.5+sin(theta)*rmAreaTilesToFraction(sqrt(realNumberNonGaiaPlayers)*4500));
   rmPlaceObjectDefAtLoc(shadeID, 0, 0.5+cos(theta)*rmAreaTilesToFraction(sqrt(realNumberNonGaiaPlayers)*4500), 0.5+sin(theta)*rmAreaTilesToFraction(sqrt(realNumberNonGaiaPlayers)*4500));
   rmPlaceObjectDefAtLoc(shadeID, 0, 0.5+cos(theta)*rmAreaTilesToFraction(sqrt(realNumberNonGaiaPlayers)*4500), 0.5+sin(theta)*rmAreaTilesToFraction(sqrt(realNumberNonGaiaPlayers)*4500));
...
for(i=1; <realNumberPlayers) {
   theta = rmDegreesToRadians((360 / (realNumberPlayers - 1)) * i);
   rmPlaceObjectDefAtLoc(fixedPortalID, 0, 0.5+cos(theta)*rmAreaTilesToFraction(sqrt(realNumberNonGaiaPlayers)*3000), 0.5+sin(theta)*rmAreaTilesToFraction(sqrt(realNumberNonGaiaPlayers)*3000));
}

    rmPlaceObjectDefInArea(bonusGoldID, 0,  rmAreaID("bonus island"+0), realNumberNonGaiaPlayers * 2 );
    //rmPlaceObjectDefInArea(titanID, realNumberPlayers,  rmAreaID("bonus island"+0), 5 );
   ...
   for(i=1; <bonusCount+1)
   {
      ...
      rmSetAreaTerrainType(bonusIslandID, "CliffNorseB"); 
      rmSetAreaWarnFailure(bonusIslandID, false);
/*      rmAddAreaConstraint(bonusIslandID, playerEdgeConstraint); */
   ...
   for(i=1; <realNumberPlayers*10)
   {
      ...
      rmSetAreaTerrainType(id3, "NorseRoadA");
...
rmPlaceObjectDefPerPlayer(farPredatorID, false);
  ...
  // Text
   ...
   int avoidRock=rmCreateTypeDistanceConstraint("avoid rock", "skull decoration", 8.0);
   int grassID=rmCreateObjectDef("grass");
   rmAddObjectDefItem(grassID, "mist", 1, 1.0);
   ...
   rmPlaceObjectDefAtLoc(grassID, 0, 0.5, 0.5, 30*realNumberNonGaiaPlayers);
   
   int rockID2=rmCreateObjectDef("rock group");
   rmAddObjectDefItem(rockID2, "Rock snow", 1, 1.0);
   ...
   rmPlaceObjectDefAtLoc(rockID2, 0, 0.5, 0.5, 3*realNumberNonGaiaPlayers); 


   int avoidRuins=rmCreateTypeDistanceConstraint("avoid ruins", "ruins", 8.0);
   int ruinsID2=rmCreateObjectDef("ruins  group");
   rmAddObjectDefItem(ruinsID2, "Runestone", 1, 2.0);
   ...
   rmPlaceObjectDefAtLoc(ruinsID2, 0, 0.5, 0.5, 3*realNumberNonGaiaPlayers); 

   int stagID2=rmCreateObjectDef("stag group");
   rmAddObjectDefItem(stagID2, "Skeleton Giant", 1, 2.0);
   ...
   rmPlaceObjectDefAtLoc(stagID2, 0, 0.5, 0.5, 2*realNumberNonGaiaPlayers*realNumberNonGaiaPlayers); 
