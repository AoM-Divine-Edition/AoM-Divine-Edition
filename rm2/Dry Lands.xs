...
void main(void)
{
	...
	if(rmRandFloat(0,1)<0.75)
		rmAddObjectDefItem		(IDStartingHunt, "lizard", rmRandInt(4,6), 14.0);
	else
		rmAddObjectDefItem		(IDStartingHunt, "lizard", 3, 12.0);
	...
	rmAddObjectDefItem			(IDMediumDeer, "boar", rmRandInt(3,8), 4.0);
	...
	if(bonusChance < 0.33)   
		rmAddObjectDefItem		(IDBonusHunt, "Lizard", rmRandInt(4,8), 10.0);
	else if(bonusChance < 0.66)
		rmAddObjectDefItem		(IDBonusHunt, "Lizard", rmRandInt(4,8), 10.0);
	else
		rmAddObjectDefItem		(IDBonusHunt, "Lizard", rmRandInt(4,8), 12.0);
	...
	if(cMapSize == 2){
		...
		rmAddObjectDefItem(giantHuntableID, "boar", rmRandInt(5,6), 4.0);
		...
		rmAddObjectDefItem(giantHuntable3ID, "boar", rmRandInt(5,6), 4.0);
		...
		rmAddObjectDefItem(giantHuntable2ID, "boar", rmRandInt(3,4), 4.0);
	...
	for(i=1; <cNumberPlayers)
	{
		...
		rmSetAreaTerrainType	(AreaPlayer, "DryLand");
		rmAddAreaTerrainLayer	(AreaPlayer, "DryLand", 8, 20);
		rmAddAreaTerrainLayer	(AreaPlayer, "DirtA", 0, 8);
	}
	...
	for(i=1; <cNumberPlayers)
	{
		...
		rmSetAreaTerrainType	(AreaPlayerInner, "DryLand");
	...
	for(i=1; <cNumberPlayers*20*mapSizeMultiplier)
	{
		...
		rmSetAreaTerrainType	(AreaGrass, "DirtA");
	...
	for(i=1; <cNumberPlayers*12*mapSizeMultiplier)
	{
		...
		rmSetAreaTerrainType	(AreaGrassDirt, "DryLand");
	...
	for(i=0; <numTries)
	{
		...
		rmSetAreaSize			(IDCliff, rmAreaTilesToFraction(550), rmAreaTilesToFraction(850));
		rmSetAreaCliffType		(IDCliff, "CliffB");
		...
		rmSetAreaTerrainType	(IDCliff, "GrassDirt75");
		rmSetAreaCliffHeight	(IDCliff, 7, 1.0, 1.0);
		rmSetAreaMinBlobDistance(IDCliff, 20.0);
		rmSetAreaMaxBlobDistance(IDCliff, 45.0);
	...
	for(i=0; <2*cNumberNonGaiaPlayers*mapSizeMultiplier)
	{
		...
		rmSetAreaTerrainType	(IDRuin, "GrassDirt75");
		rmAddAreaTerrainLayer	(IDRuin, "GrassDirt75", 0, 1);
		rmSetAreaMinBlobs		(IDRuin, 1);
		rmSetAreaMaxBlobs		(IDRuin, 3);
		...
		rmAddObjectDefItem			(IDColumn, "grass", rmRandInt(0,1), 0.0);
		rmAddObjectDefItem			(IDColumn, "rock dirt small", rmRandInt(1,3), 4.0);
		rmAddObjectDefItem			(IDColumn, "rock dirt big", rmRandInt(6,12), 6.0);
	...
	for(i=0; <numTries)
	{
		...
		rmSetAreaForestType		(IDForest, "mixed palm forest");
	...
	int IDCactus					= rmCreateObjectDef("LargeCactus");
	rmAddObjectDefItem			(IDCactus, "LargeCactus", rmRandInt(2,6), 6.0);
	rmSetObjectDefMinDistance	(IDCactus, 0.0);
	rmSetObjectDefMaxDistance	(IDCactus, rmXFractionToMeters(0.5));
	rmAddObjectDefConstraint	(IDCactus, AvoidAll);
	rmAddObjectDefConstraint	(IDCactus, AvoidImpassableLand);
	rmAddObjectDefConstraint	(IDCactus, AvoidStartingSettleShort);
	rmPlaceObjectDefAtLoc		(IDCactus, 0, 0.5, 0.5, 3*cNumberNonGaiaPlayers*mapSizeMultiplier);
