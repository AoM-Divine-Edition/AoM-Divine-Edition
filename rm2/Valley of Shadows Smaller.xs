...
void main(void)
{
	...
	rmxInit("Valley of Shadows (by AProperGentleman)", false, false, false);
...
int realNumberNonGaiaPlayers = cNumberNonGaiaPlayers - 1;
  ...
  // Text
   ...
   rmTerrainInitialize("CliffEgyptianA", 30.0);
   ...
   int shortAvoidSettlement=rmCreateTypeDistanceConstraint("short avoid settlement", "AbstractSettlement", 15.0);
   ...
   int veryShortAvoidImpassableLand=rmCreateTerrainDistanceConstraint("short avoid impassable land", "Land", false, 1.0);
   ...
   rmAddObjectDefItem(startingGoldID, "Gold mine tiny", 1, 0.0);
   ...
   if(waterBuffaloNumber<0.3)
      rmAddObjectDefItem(closeWaterBuffaloID, "baboon", 2, 4.0);
   else if(waterBuffaloNumber<0.6)
      rmAddObjectDefItem(closeWaterBuffaloID, "baboon", 4, 4.0);
   else 
      rmAddObjectDefItem(closeWaterBuffaloID, "baboon", 8, 6.0);
   ...
   rmAddObjectDefItem(mediumGoldID, "gold mine small", 1, 4.0);
   ...
   rmAddObjectDefItem(titanID, "Obelisk", 1, 0.0);   
   rmAddObjectDefItem(titanID, "Statue Pharaoh", 1, 5.0);   
   int pyramidID=rmCreateObjectDef("pyramid");
   rmAddObjectDefItem(pyramidID, "pyramid small", 1, 0.0);
   ...
   rmAddObjectDefConstraint(bonusGoldID, veryShortAvoidImpassableLand);
   ...
   if(predatorSpecies<0.5)   
      rmAddObjectDefItem(farPredatorID, "gazelle", 6, 4.0);
   else
      rmAddObjectDefItem(farPredatorID, "zebra", 8, 4.0);
   ...
   if(bonusChance<0.5)   
      rmAddObjectDefItem(bonusHuntableID, "zebra", 5, 4.0);
   else
      rmAddObjectDefItem(bonusHuntableID, "gazelle", 5, 8.0);
   ...
   rmAddObjectDefItem(randomTreeID, "palm tree", 1, 0.0);
...
rmSetAreaLocation(bonusIslandID, 0.5, 0.5);
    ...
    rmSetAreaTerrainType(bonusIslandID, "SandD"); 
   ...
   rmBuildAllAreas();
    ...
    rmPlaceObjectDefInArea(titanID, 0,  rmAreaID("bonus island"+0), 6 );
    rmPlaceObjectDefInArea(pyramidID, 0,  rmAreaID("bonus island"+0), 1 );
...
for (i=1; < realNumberPlayers) {
      rmPlaceObjectDefInArea(mediumGoldID, 0,  rmAreaID("player"+i), 1);
   }
   ...
   for(i=1; <realNumberPlayers * 3)
	 ...
	 rmPlaceObjectDefInArea(farGoldID, 0,  rmAreaID("bonus island"+0));
...
//      rmPlaceObjectDefInArea(farGoldID, 0, rmAreaID("player"+i), 2);
   ...
   for(i=1; <realNumberPlayers * 3)
   rmPlaceObjectDefInArea(bonusGoldID, 0,  rmAreaID("bonus island"+0));
   ...
   for (i=1; < realNumberNonGaiaPlayers * 3) {
      rmPlaceObjectDefInRandomAreaOfClass(relicID, 0, classBonusIsland);
   }

for (i=1; < realNumberNonGaiaPlayers ) {
     rmPlaceObjectDefInArea(relicID, 0,  rmAreaID("bonus island"+0));
   }
...
rmPlaceObjectDefPerPlayer(farPredatorID, false);
  ...
  // Text
   ...
   rmAddObjectDefItem(grassID, "sand drift dune", 1, 1.0);
   ...
   rmAddObjectDefItem(rockID2, "bush", 3, 1.0);
   ...
   rmAddObjectDefItem(stagID2, "dust devil", 1, 1.0);
   ...
   rmPlaceObjectDefAtLoc(stagID2, 0, 0.5, 0.5, 10*realNumberNonGaiaPlayers); 
   ...
   for(i=1; <realNumberPlayers * 2)
	rmPlaceObjectDefInArea(snakeID2, 0,  rmAreaID("bonus island"+0), 1);
