...
void main(void)
{
	...
	rmxInit("Oblivion Forest (by AProperGentleman)", false, false, false);
...
int shortIslandConstraint=rmCreateClassDistanceConstraint("forests avoid islands", classIsland, 1.0);
   ...
   if(waterBuffaloNumber<0.3)
      rmAddObjectDefItem(closeWaterBuffaloID, "Berry Bush", 2, 4.0);
   else if(waterBuffaloNumber<0.6)
      rmAddObjectDefItem(closeWaterBuffaloID, "Berry Bush", 3, 4.0);
   else 
      rmAddObjectDefItem(closeWaterBuffaloID, "Berry Bush", 4, 6.0);
   rmSetObjectDefMinDistance(closeWaterBuffaloID, 30.0);
   rmSetObjectDefMaxDistance(closeWaterBuffaloID, 50.0);

   int stragglerTreeID=rmCreateObjectDef("straggler tree");
   rmAddObjectDefItem(stragglerTreeID, "marsh tree", 3, 0.0);
   ...
   rmAddObjectDefItem(titanID, "berry bush", 5, 3.0);
   ...
   if(predatorSpecies<0.5)   
      rmAddObjectDefItem(farPredatorID, "deer", 4, 4.0);
   else
      rmAddObjectDefItem(farPredatorID, "deer", 8, 4.0);
   ...
   rmAddObjectDefItem(randomTreeID, "marsh tree", 1, 0.0);
  ...
  rmPlacePlayer(cNumberNonGaiaPlayers, -100.0, -100.0);
...
rmSetAreaLocation(bonusIslandID, 0.5, 0.5);
    ...
    rmSetAreaTerrainType(bonusIslandID, "MarshD"); 
   ...
   rmBuildAllAreas();
    ...
    rmPlaceObjectDefInArea(titanID, 0,  rmAreaID("bonus island"+0), 25 );
...
/*      rmAddAreaConstraint(bonusIslandID, playerEdgeConstraint); */
   ...
   for(i=1; <realNumberPlayers*10)
   {
	...
	for (i = 0; < 1000*(realNumberPlayers+cMapSize))
	{
	int forestID=rmCreateArea("wallforest"+i);
         rmSetAreaSize(forestID, rmAreaTilesToFraction(25), rmAreaTilesToFraction(50));
         rmSetAreaWarnFailure(forestID, false);
         rmSetAreaForestType(forestID, "marsh forest");
         rmAddAreaConstraint(forestID, shortIslandConstraint);
         ...
         rmSetAreaMinBlobDistance(forestID, 0.0);
         rmSetAreaMaxBlobDistance(forestID, 0.0);
...
rmPlaceObjectDefPerPlayer(farPredatorID, false);
  ...
  // Text
   ...
   for(i=0; <bonusCount + 1)
   {
      forestCount=rmRandInt(4, 7);

	if (i == 0) {
		forestCount = 3 * forestCount;
	} else {
   ...
   rmAddObjectDefItem(rockID2, "Rock Limestone Small", 3, 1.0);
   ...
   rmAddObjectDefItem(ruinsID2, "ruins", 1, 2.0);
   ...
   rmAddObjectDefItem(stagID2, "gaia forest tree", 1, 2.0);
