...
void main(void){
	...
	string terrainLush = "JungleB";
	
	string terrainLargeAreaEdge = terrainBase;
	string terrainLargeAreaMid = "JungleDirt25";
	string terrainLargeAreaStart = "JungleDirt50";
	string terrainLargeAreaCentre = "JungleDirt75";
	
	string terrainBeauty = "jungle b"; //"JungleDirt"; <--- Causes Crash?
	string waterType = "Marsh Pool";
	string forestType = "Jungle Forest";
	...
	int playerCore = -1;
	for(i=1; <cNumberPlayers){
		playerCore = rmCreateArea("PlayerCore"+i);
		rmSetAreaSize(classPlayerCore, 0.9*playerFraction, 1.1*playerFraction);
		rmAddAreaToClass(id, classPlayer);
		...
		rmAddAreaConstraint(playerCore, playerConstraint);
	...
	if(rmRandFloat(0.0,1.0) > 0.5){
		...
		rmAddObjectDefItem(fishiesID, "Fish - Salmon", 1, 3.0);
		...
		waterReedID=rmCreateObjectDef("Water Reeds");
		rmAddObjectDefItem(waterReedID, "Water Reeds", 1, 1.0);
		...
		for(i=0; <numPond){
			...
			rmSetAreaSize(smallPondID, rmAreaTilesToFraction(500), rmAreaTilesToFraction(500));
	...
	rmAddObjectDefItem(closePigID, "pig", rmRandInt(2,5), 2.0);
	...
	rmAddObjectDefItem(startingDuckID, "Duck", rmRandInt(5,10), 3.0);
	...
	rmAddObjectDefItem(stragglerTreeID, "Jungle Tree", 1, 0.0);
	...
	rmAddObjectDefItem(mediumPigID, "pig", rmRandFloat(2,3), 4.0);
	...
	if(bonusChance < 0.5){
		rmAddObjectDefItem(bonusHuntableID, "boar", 4, 4.0);
	} else {
		rmAddObjectDefItem(bonusHuntableID, "Elephant Indian", rmRandInt(3,4), 4.0);
	}
	...
	if(predatorSpecies<0.4){
		rmAddObjectDefItem(farPredatorID, "lizard", 2.0, 4.0);
	} else {
		rmAddObjectDefItem(farPredatorID, "Tiger", rmRandInt(1,2), 4.0);
		//rmAddObjectDefItem(farPredatorID, "Hyena", 2, 4.0); //makeshift Tiger
	}
	...
	rmAddObjectDefItem(randomTreeID, "Jungle tree", 1, 0.0);
